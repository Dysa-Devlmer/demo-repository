...
export class DashboardSnapshotController {
  ...
  async create(@Query('type') type?: SnapshotType, @Query('triggeredBy') triggeredBy?: string) {
    return this.snapshotService.createSnapshot(type || SnapshotType.DAILY, triggeredBy || 'manual');
  }
