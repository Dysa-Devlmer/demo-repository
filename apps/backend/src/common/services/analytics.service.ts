...
export class AnalyticsService {
  ...
  getDashboardMetrics(): DashboardMetrics {
    ...
    const todayMetrics = this.metricsCache.filter(
    ...
    const customerMetrics = todayMetrics.filter(
    ...
    const todayCustomers = new Set(
      customerMetrics.map((m) => m.tags?.customerId),
    ).size;
  ...
  async generateReport(
    ...
    const revenueMetrics = periodMetrics.filter(
      (m) => m.category === "revenue",
    );
    const orderMetrics = periodMetrics.filter((m) => m.category === "orders");
    const customerMetrics = periodMetrics.filter(
      (m) => m.category === "customers",
    );
  ...
  async generateDailyReport(): Promise<void> {
    if (
      !this.config.reportGeneration.enabled ||
      !this.config.reportGeneration.scheduleDaily
    ) {
      return;
    }
  ...
  async generateWeeklyReport(): Promise<void> {
    if (
      !this.config.reportGeneration.enabled ||
      !this.config.reportGeneration.scheduleWeekly
    ) {
      return;
    }
  ...
  async generateMonthlyReport(): Promise<void> {
    if (
      !this.config.reportGeneration.enabled ||
      !this.config.reportGeneration.scheduleMonthly
    ) {
      return;
    }
  ...
  async cleanupOldData(): Promise<void> {
    ...
    this.reportsCache = this.reportsCache.filter(
      (r) => new Date(r.generatedAt) > cutoffDate,
    );
