...
export class NotificationsController {
  ...
  async sendBulkNotification(
    ...
    const result = await this.notificationService.sendBulkNotification(recipients, payload);
  ...
  @ApiBody({
    schema: {
      ...
      required: ['customerEmail', 'customerName', 'orderNumber', 'items', 'total'],
    },
  })
  @ApiResponse({ status: 200, description: 'Order confirmation sent' })
  async sendOrderConfirmation(@Body() orderData: any) {
    const success = await this.notificationService.sendOrderConfirmation(orderData);
    return {
      success,
  ...
  async sendReservationConfirmation(@Body() reservationData: any) {
    const success = await this.notificationService.sendReservationConfirmation(reservationData);
    return {
      success,
      message: success
  ...
  async sendWeeklySummary(@Body() data: { recipients: string[]; businessData: any }) {
    ...
    return {
      success,
      message: success ? 'Weekly summary sent' : 'Failed to send weekly summary',
      timestamp: new Date().toISOString(),
    };
  }
  ...
  async testNotificationChannel(
    ...
    const success = await this.notificationService.sendNotification(testPayload);
