...
export class BackupController {
  ...
  async createManualBackup(@Body() createBackupDto: { type: 'database' | 'files' | 'full' }) {
    try {
      const backup = await this.backupService.triggerBackup(createBackupDto.type);
  ...
  async getDiskUsage() {
    ...
    return {
      ...
      backupCount: 15,
      warning: (usagePercentage) => (usagePercentage > 80 ? 'High disk usage detected' : null),
      timestamp: new Date().toISOString(),
    };
  }
}
