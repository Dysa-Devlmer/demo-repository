...
export class SecurityGuard implements CanActivate {
    ...
    for (const value of toCheck) {
      for (const pattern of this.suspiciousPatterns) {
        if (pattern.test(value)) {
          throw new ForbiddenException(`Suspicious pattern detected: ${pattern}`);
        }
      }
    }
    ...
    if (request.body && typeof request.body === 'object') {
      ...
      for (const pattern of this.suspiciousPatterns) {
        if (pattern.test(bodyString)) {
          throw new ForbiddenException(`Suspicious pattern in request body: ${pattern}`);
        }
      }
    }
  }
  ...
  private checkAttackVectors(request: Request): void {
    ...
    const suspiciousParams = ['eval', 'exec', 'system', 'shell_exec', 'passthru'];
