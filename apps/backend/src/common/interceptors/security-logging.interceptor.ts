...
export class SecurityLoggingInterceptor implements NestInterceptor {
  ...
  private logSecurityEvent(event: SecurityEvent): void {
    ...
    const isCritical = event.securityFlags?.some((flag) =>
      criticalFlags.includes(flag),
    );
    const isAuthFailure = event.securityFlags?.includes(
    ...
    if (isCritical) {
      ...
      this.logger.warn(
        `🔒 Authentication failure: ${event.ip} -> ${event.url}`,
      );
    } else if (isRateLimit) {
      ...
      this.logger.warn(
        `⚠️  HTTP Error ${event.statusCode}: ${event.ip} -> ${event.url}`,
      );
    } else {
  ...
  private alertSecurityTeam(event: SecurityEvent): void {
    ...
    this.logger.error(
      `🚨 SECURITY ALERT TRIGGERED: ${JSON.stringify(event, null, 2)}`,
    );
