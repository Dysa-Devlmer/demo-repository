...
export class SettingsController {
  ...
  async testWhatsAppMessage(@Body() body: { to: string; message?: string }) {
    try {
      ...
      const result = await this.whatsappService.sendTextMessage(body.to, message);
  ...
  async testWhatsAppMenu(@Body() body: { to: string }) {
    try {
      ...
      throw new HttpException('Error al enviar menú de prueba', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
