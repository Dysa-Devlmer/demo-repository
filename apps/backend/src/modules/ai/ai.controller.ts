...
export class AiController {
  ...
  async getHealth() {
    try {
      ...
      throw new HttpException('AI service health check failed', HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
  ...
  async chat(@Body() chatDto: ChatDto): Promise<ChatResponse> {
    ...
    try {
      ...
      const response = await this.generateEnterpriseAIResponse(chatDto.message, {
      ...
      return {
        response:
          'Gracias por contactarnos. Nuestro equipo de atención al cliente está disponible para ayudarte. ¿En qué podemos asistirte hoy?',
        sessionId: chatDto.sessionId || `session_${Date.now()}`,
        timestamp: new Date().toISOString(),
  ...
  private async generateEnterpriseAIResponse(message: string, context: any): Promise<string> {
    try {
      ...
      const response = await this.ollamaService.generateRestaurantResponse(message, {
  ...
  private generateHardcodedResponse(message: string, context: any): string {
    ...
    if (
      ...
      normalizedMessage.includes('carta') ||
      normalizedMessage.includes('plato')
    ) {
      if (context.menuItems && context.menuItems.length > 0) {
        const menuList = context.menuItems
          .map((item: MenuItemContext) => `${item.name} ($${item.price})`)
          .join(', ');
    ...
    if (
      (normalizedMessage.includes('caro') || normalizedMessage.includes('precio')) &&
      context.menuItems &&
      context.menuItems.length > 0
    ) {
      const mostExpensive = context.menuItems.reduce(
        (max: MenuItemContext, item: MenuItemContext) => (item.price > max.price ? item : max),
        context.menuItems[0]
      );
    ...
    if (
      ...
      normalizedMessage.includes('domicilio') ||
      normalizedMessage.includes('llevar')
    ) {
    ...
    if (
      ...
      normalizedMessage.includes('buenos') ||
  ...
  async getModels() {
    try {
      ...
      throw new HttpException('Failed to retrieve models', HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
  ...
  async analyzeMessage(@Body() body: { message: string }) {
    try {
      ...
      throw new HttpException('Failed to analyze message', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async exportQTable() {
    try {
      ...
      throw new HttpException('Failed to export Q-Table', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async importQTable(@Body() body: { qTable: Record<string, Record<string, number>> }) {
    try {
      ...
      throw new HttpException('Failed to import Q-Table', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
