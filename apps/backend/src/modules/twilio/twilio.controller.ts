...
export class TwilioController {
  ...
  @Post('voice/webhook')
  async handleVoiceWebhook(@Body() body: TwilioWebhookData, @Res() res: Response) {
    try {
      this.logger.log('Received Twilio voice webhook:', JSON.stringify(body));

      const processed: ProcessedWebhook = this.twilioService.processVoiceWebhook(body);
      ...
      switch (processed.type) {
        case 'reservation':
          if (processed.data.recordingUrl) {
            ...
            this.logger.log(`Processing reservation recording: ${processed.data.recordingUrl}`);
            ...
            const aiResponse = await this.ollamaService.generateRestaurantResponse(
              `Cliente solicita reserva: ${transcription}`,
              {
                restaurantInfo: {
                  ...
                  specialties: ['Reservas', 'Mesa para parejas', 'Eventos especiales'],
                },
              }
            );

            // Notify admin panel
            if (processed.data.from && processed.data.callSid && processed.data.recordingUrl) {
  ...
  <Hangup/>
        ...
        case 'order':
          if (processed.data.recordingUrl) {
            ...
            this.logger.log(`Processing order recording: ${processed.data.recordingUrl}`);
            const transcription = await this.twilioService.transcribeRecording(
              processed.data.recordingUrl
            );
            ...
            const aiResponse = await this.ollamaService.generateRestaurantResponse(
              `Cliente hace pedido: ${transcription}`,
              {
                restaurantInfo: {
                  name: 'DysaBot Restaurant',
                  specialties: ['Delivery', 'Comida rápida', 'Pedidos telefónicos'],
                },
              }
            );
...
<Response>
  ...
  async makeCall(@Body() options: VoiceCallOptions) {
    try {
      ...
      if (result.success) {
        ...
        throw new HttpException(result.error || 'Failed to make call', HttpStatus.BAD_REQUEST);
      }
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
  ...
  @Post('send-sms')
  async sendSMS(@Body() body: { to: string; message: string; mediaUrl?: string[] }) {
    try {
      ...
      if (result.success) {
        ...
        throw new HttpException(result.error || 'Failed to send SMS', HttpStatus.BAD_REQUEST);
      }
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
  ...
  async getCallStatus(@Body('callSid') callSid: string) {
    try {
      ...
      throw new HttpException('Failed to get call status', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async hangupCall(@Body('callSid') callSid: string) {
    try {
      ...
      if (success) {
        ...
        throw new HttpException('Failed to hangup call', HttpStatus.BAD_REQUEST);
      }
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      this.logger.error('Hangup call error:', err.message);
      throw new HttpException('Failed to hangup call', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async sendReservationSMS(
    ...
    try {
      ...
      throw new HttpException('Failed to send reservation SMS', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async sendOrderSMS(
    ...
    try {
      const result = await this.twilioService.sendRestaurantNotificationSMS(body.to, 'order', {
      ...
      throw new HttpException('Failed to send order SMS', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
...
<Response>
  ...
  <Hangup/>
      ...
      throw new HttpException('Failed to make test call', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
